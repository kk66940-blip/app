...
] if search else rap_items

# ==================== FUNGSI TAMPILAN HIERARCHY + EDIT + HAPUS ====================
def show_rap_hierarchy_with_actions(items):
        ...
        with st.expander(f"{indent}{desc}", expanded=(level <= 1)):
            ...
            with btn_col1:
                if st.button("✏️ Edit Harga", key=f"edit_{item['id']}", use_container_width=True):
                    ...
                    st.rerun()
            with btn_col2:
                if st.button("🗑️ Hapus", key=f"delete_{item['id']}", use_container_width=True):
                    if st.session_state.get("confirm_delete") == item['id']:
                        supabase.table("rap_items").delete().eq("id", item['id']).execute()
...
# Tampilkan hierarchy
show_rap_hierarchy_with_actions(filtered)

# ==================== TOTAL RAP ====================
if filtered:
    total_volume = sum((item.get('volume', 0) or 0) for item in filtered)
    total_planned = sum((item.get('volume', 0) or 0) * (item.get('planned_price', 0) or 0) for item in filtered)
    total_execution = sum((item.get('volume', 0) or 0) * (item.get('execution_price', 0) or 0) for item in filtered)
    total_upah = sum((item.get('upah', 0) or 0) for item in filtered)
    ...
    col2.metric("Total Harga Rencana", f"Rp {total_planned:,.0f}")
    col3.metric("Total Harga Pelaksanaan", f"Rp {total_execution:,.0f}")
