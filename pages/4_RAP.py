...
def build_rap_tree(items):
    """Membangun struktur tree berdasarkan parent_id (lebih robust daripada string matching)"""
...
def get_root_items(items):
    """Ambil item root (tidak punya parent atau parent tidak ada di data)"""
...
def display_rap_like_rab(items):
    """Tampilan RAP menggunakan parent_id (lebih stabil & konsisten dengan RAB)"""
    if not items:
        return
