...
def display_rap_tree(
    ...
    """
    RAP Tree dengan dukungan hierarchy di mana parent_id merujuk ke rab_items.id
    (bukan self-referencing di rap_items).
    """
    if not items:
        st.info("Tidak ada data untuk ditampilkan.")
        return
    ...
    # Group berdasarkan parent_id (ID dari rab_items)
    children_map: Dict[Optional[int], List[Dict]] = defaultdict(list)
    for item in items:
        children_map[item.get('parent_id')].append(item)
